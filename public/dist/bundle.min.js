...
(function () {
  ...
  var DotGain = /*#__PURE__*/function (_Gallery) {
    ...
    _createClass(DotGain, [{
      ...
      value: function _onImageLoad(event, image) {
        ...
        if (this.get('imgStyle').size === 'cover') {
          console.log("cover mode dotgain");
          if (event.target.getBBox().width > event.target.getBBox().height) {
            event.target.setAttributeNS(null, "width");
            event.target.setAttributeNS(null, "height");
          }
        }
    ...
    _createClass(Canvas, [{
      ...
      value: function showImage(cImage, src) {
        ...
        image.onload = function (event) {
          ...
          if (_this.preferences.imgStyle.objectFit === 'contain') {
            width = s1.w <= width && s1.h <= height ? s1.w : s2.w;
            height = width / image.width * image.height;
          } else if (_this.preferences.imgStyle.objectFit === 'cover') {
            width = s1.w <= width && s1.h <= height ? s2.w : s1.w;
            height = width / image.width * image.height;
          }
          image.x1 = (canvas.width - width) * _this.preferences.imgStyle.alignment.x;
          image.y1 = (canvas.height - height) * _this.preferences.imgStyle.alignment.y;
          image.w = width;
          image.h = height;
          context.drawImage(event.target, (canvas.width - width) * _this.preferences.imgStyle.alignment.x, (canvas.height - height) * _this.preferences.imgStyle.alignment.y, width, height);
  ...
  var Pipe = /*#__PURE__*/function (_Canvas) {
    _inherits(Pipe, _Canvas);
    var _super = _createSuper(Pipe);
    function Pipe(albums) {
      var _this;
