  ...
  var Canvas = /*#__PURE__*/function (_Gallery) {
    ...
    _createClass(Canvas, [{
      ...
      value: function drawImage() {
        ...
        var width = this.canvasContainer.clientWidth * scale;
        var height = this.canvasContainer.clientHeight * scale;
  ...
  var Transitions = /*#__PURE__*/function (_Canvas) {
    ...
    _createClass(Transitions, [{
      ...
      value: function transitionDotGain(canvas) {
        ...
        var grid = {
          x: this.get('grid').x || this.get('grid', 30),
          y: this.get('grid').y || this.get('grid', 30)
        };
        var pending = 0;
        for (var x = 0; x <= canvas.width / grid.x + 1; x += 1) {
          for (var y = 0; y <= canvas.height / grid.y + 1; y += 1) {
            context.moveTo(0, 0);
            var r = -calculateDistance(x * grid.x, y * grid.y, this.preferences.origin.x, this.preferences.origin.y) * this.get('transitionDuration') / this.get('fps') + this.get('transitionEase', 0) * this.transitionFrame + this.transitionFrame / this.get('fps') * Math.abs(grid.x, grid.y);
